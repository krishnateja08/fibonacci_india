...
# Sector map — used to build the TICKERS universe below and to label each
# stock's GICS sector (via its SPDR sector-ETF proxy). Edit freely; TICKERS
# is derived from this dict's keys so you only maintain the list in one place.
SECTOR_MAP = {
    **{s: "XLK" for s in [
        # Technology (17)
...
# Ticker universe = every symbol in SECTOR_MAP, in insertion order.
TICKERS: List[str] = list(SECTOR_MAP.keys())

# Major US benchmarks + the 11 SPDR sector ETFs (same ETFs used as the
# sector proxies in SECTOR_MAP above), analyzed the same way as individual
# stocks but kept in a separate list/tab — index Fibonacci levels are a
# macro/sector context read, not something you'd place an entry order
# against the way you would an individual stock.
